...
<section class="content">
	...
	<div class="row">
		<div class="col-md-12">
			...
			<?php echo form_open_multipart(base_url().'admin/popular/update/'.$popular->popular_id,array('class' => 'form-horizontal')); ?>
            ...
            <div class="form-group <?=form_error('popular_product') ? 'has-error' : null?>">
                ...
                <select name="popular_product" class="form-control select2" id="popular_product">
                  ...
                  <?php foreach ($product as $x): ?>
                    <option value="<?=$x->product_id?>" <?=($popular->popular_product == $x->product_id ? 'selected' : '')?> ><?=$x->product_name?></option>
                  <?php endforeach; ?>
                </select>
                <span class="help-block"><i><small><?=form_error('popular_product')?></small></i></span>
						...
						<div class="form-group <?=form_error('popular_order') ? 'has-error' : null?>">
							...
							<div class="col-sm-4">
                <input type="number" class="form-control" name="popular_order" value="<?=$popular->popular_order?>">
