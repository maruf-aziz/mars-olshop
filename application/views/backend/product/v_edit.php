...
<section class="content">
	...
	<div class="row">
		<div class="col-md-12">
			...
			<?php echo form_open_multipart(base_url().'admin/product/update/'.$product->product_id,array('class' => 'form-horizontal')); ?>
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group <?=form_error('product_name') ? 'has-error' : null?>">
							...
							<div class="col-sm-6">
                <input type="text" class="form-control" name="product_name" value="<?=$product->product_name?>" autocomplete="off">
                ...
                <select name="product_category" class="form-control select2" id="product_category">
                  ...
                  <?php foreach ($category as $x): ?>
                    <option value="<?=$x->category_id?>" <?=($product->product_category == $x->category_id ? 'selected' : '')?> ><?=$x->category_name?></option>
                  <?php endforeach; ?>
                </select>
                <span class="help-block"><i><small><?=form_error('product_category')?></small></i></span>
						...
						<div class="form-group <?=form_error('product_desc') ? 'has-error' : null?>">
							...
							<div class="col-sm-9">
								<textarea class="form-control editor" name="product_desc"><?=$product->product_desc?></textarea>
                <span class="help-block"><i><small><?=form_error('product_desc')?></small></i></span>
						...
						<div class="form-group <?=form_error('product_price') ? 'has-error' : null?>">
							...
							<div class="col-sm-4">
                <input type="text" class="form-control" name="product_price" id="field-price" value="<?=$product->product_price?>" autocomplete="off">
                <span class="help-block"><i><small><?=form_error('product_price')?></small></i></span>
							</div>
						</div>

						<div class="form-group <?=(isset($message)) ? 'has-error' : null ?>">
							<label for="" class="col-sm-2 control-label">New Images <span></span></label>
							<div class="col-sm-2">
                ...
                <span class="help-block"><i><small><?=(isset($message))? $message : ""; ?></small></i></span>
						...
						<div class="form-group <?=form_error('product_stock') ? 'has-error' : null?>">
							...
							<div class="col-sm-4">
                <input type="number" class="form-control" name="product_stock" value="<?=$product->product_stock?>" autocomplete="off">
                ...
                <select name="product_status" class="form-control select2" id="product_status">
                  ...
                  <option value="pre order" <?=($product->product_status == 'pre order' ? 'selected' : '')?>>Pre Order</option>
